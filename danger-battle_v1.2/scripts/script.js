window.addEventListener('DOMContentLoaded', () => {
    ...
    let lifeCountDanger = document.querySelector('.lifeCountDanger'),
    ...
    chooseCharacter__elements.forEach(item => {
        item.addEventListener('click', (e) => {
            ...
            if(character1 === true) {
                ...
                const title = document.createElement('div');
                title.classList.add('title');
                title.innerHTML = 'Вы играете за персонажа под ником "Biker!"';
                container__title.append(title);
            }
        });
    });
    ...
    function randomPunch(randomNumber, lifeCountDanger, lifeCountBiker, gameOver, YouLose) {
        ...
        const lifeCountForAcute = Math.floor(Math.random() * (25 - 5)) + 5; // Число выбирается рандомно от 5 до 25
